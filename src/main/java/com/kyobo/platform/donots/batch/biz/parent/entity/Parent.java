...
public class Parent {
    ...
    private LocalDateTime lastActivityIndicatorAggregatedDatetime;              // 지난 활동지표 집계시각
...
//    private List<NotifMethod> notifMethods = new ArrayList<>();
    ...
    // 아니면 유효성 체크로 다 걸러낼까?
    public void deepCopyExceptKeyFrom(Parent parent) {
        ...
        if (parent.getPageviewedCount() != null)   this.pageviewedCount = parent.getPageviewedCount();
        if (parent.getRecipePostingCount() != null)   this.recipePostingCount = parent.getRecipePostingCount();
        if (parent.getScrapbookedCount() != null)   this.scrapbookedCount = parent.getScrapbookedCount();
        if (parent.getReactionAddingCount() != null)   this.reactionAddingCount = parent.getReactionAddingCount();
        if (parent.getScrapbookingCount() != null)   this.scrapbookingCount = parent.getScrapbookingCount();
        if (parent.getLastActivityIndicatorAggregatedDatetime() != null)   this.lastActivityIndicatorAggregatedDatetime = parent.getLastActivityIndicatorAggregatedDatetime();
    }
